...
## From #352 export-materiality quality-gate code review (non-blocking, pre-existing/plausible)

Manual review against upstream/main (`75a9f0e`) for `fix/issue-352-export-mode-materiality`, covering CLAUDE.md compliance, bug scan, call-site consistency, and git-blame context on `core/bess/inverter_controller.py`, `core/bess/growatt_min_controller.py`, `core/bess/solax_modbus_growatt_controller.py`, and `backend/api.py`. No CONFIRMED (score ≥ 80) issues found. Two low-confidence items, neither blocking:

1. **`mode_for_period()` has no bounds check** on `self.strategic_intents[period]`, unlike `_export_demoted_for_period()` which explicitly guards `0 <= period < len(self.strategic_intents)`. All current call sites already bounds-check before calling `mode_for_period` (the loop in `growatt_min_controller.py` is bounded by `len(self.strategic_intents)`; the two call sites in `solax_modbus_growatt_controller.py` are guarded by `if current_period/effective_period < len(self.strategic_intents):`), so this doesn't fire today. Matches the pre-existing pattern (the old inline `self.INTENT_TO_MODE.get(self.strategic_intents[period], ...)` had the same lack of guard) — not a regression, just worth remembering if a new unguarded call site is added later.

2. **`core/bess/simulation/inverter_simulator.py` intentionally bypasses the new demotion gate** — `derive_control_command` still calls `InverterController.INTENT_TO_MODE.get(...)` directly rather than `mode_for_period`. This is explicitly documented as intended in `docs/SOFTWARE_DESIGN.md` and `docs/agents/bess-knowledge.md` ("this is a sub-period hardware-robustness behavior invisible to the 15-minute plan/simulator"), consistent with the existing precedent for the SOLAR_EXPORT discharge gate. Flagging only so it stays a conscious choice: plan-faithfulness tests will not catch any future divergence between the simulator's mode mapping and the real controller's demotion behavior.

3. **Rebase-onto-upstream discovery (2026-07-30): `_group_periods_by_mode` was parametrized upstream to group an arbitrary candidate `intents` list (not always `self.strategic_intents`), used by `_build_candidate`/`evaluate_intents` to diff a not-yet-applied candidate schedule against what's currently on hardware.** Naively rebasing this fix's `mode = self.mode_for_period(period)` onto that would have silently ignored the passed `intents` parameter (both `mode_for_period` and `_export_demoted_for_period` hardcoded `self.strategic_intents`), making the candidate-comparison path always regroup from the currently-applied intents regardless of what candidate was passed in — i.e. `evaluate_intents` would stop detecting real schedule changes. Fixed during the rebase by threading an explicit `intents` parameter through `mode_for_period`/`_export_demoted_for_period` (defaulting to `self.strategic_intents` for the normal apply path). Residual gap, not fixed: `_export_demoted_for_period`'s energy lookup (`_planned_energy_for_period`) still reads `self.current_schedule.period_data`, which during `evaluate_intents`'s candidate check is still the *previously applied* schedule, not the candidate's own — so the demotion verdict used for the apply/no-apply *decision* can be stale for BATTERY_EXPORT periods at the comparison boundary. Not a control-safety issue (`apply_intents` always sets `self.current_schedule` to the real new schedule before rebuilding TOU for real), only a soft inaccuracy in the "should I re-write hardware" heuristic. Worth fixing properly (thread the candidate schedule's own `period_data` through `_build_candidate`) if this ever causes an observed missed/spurious re-apply.
