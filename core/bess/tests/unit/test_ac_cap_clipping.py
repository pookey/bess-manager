...
``inverter_ac_capacity_kw`` set, the optimizer must value clipped solar at
zero (so a full battery under high solar is genuinely costly) and be able to
HOLD — deliberately skip passive absorption — to preserve headroom for the
above-cap window.

Feature-off (``inverter_ac_capacity_kw = 0``) behavior must be unchanged.
"""
...
def make_capped_settings(**overrides):
    defaults = {"inverter_ac_capacity_kw": 5.0, "inverter_ac_capacity_margin": 0.0}
...
def test_effective_ac_cap_disabled_and_margin():
    ...
    derated = _effective_ac_cap_kwh(
        make_capped_settings(inverter_ac_capacity_margin=0.2), DT
    )
...
def test_hold_preserves_soe_and_classifies_as_solar_export():
    bs = make_capped_settings()
    soe = 5.0
    next_soe = _state_transition(
        soe, 0.0, bs, DT, solar_production=1.0, home_consumption=0.15, hold=True
    )
    assert next_soe == soe
...
def test_compute_charge_rate_solar_export_gate():
    capped = GrowattMinController(make_capped_settings())
    uncapped = GrowattMinController(make_battery_settings())
    control = {"grid_charge": False, "charge_rate": 100, "discharge_rate": 0}

    assert capped._compute_charge_rate("SOLAR_EXPORT", control, 0.0) == 0
    assert uncapped._compute_charge_rate("SOLAR_EXPORT", control, 0.0) == 100
    assert capped._compute_charge_rate("SOLAR_STORAGE", control, 0.0) == 100
    assert capped._compute_charge_rate("IDLE", control, 0.0) == 100


def test_simulator_hold_command_does_not_absorb_surplus():
    bs = make_capped_settings()
...
def test_simulator_solar_export_still_absorbs_without_cap():
    ...
    assert sim.period_data[0].energy.battery_charged == pytest.approx(0.85)
...
def test_clipping_day_defers_charging_and_avoids_clipping():
    ...
    cap_kwh = scenario["battery"]["inverter_ac_capacity_kw"] * 0.25
    ...
    assert "SOLAR_EXPORT" in intents_before_window  # deliberate HOLD periods
...
def test_feature_off_plan_costs_more_under_cap_reality():
    ...
    scenario_off["battery"]["inverter_ac_capacity_kw"] = 0.0
