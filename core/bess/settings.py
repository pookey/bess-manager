...
INVERTER_AC_CAPACITY_KW = 0.0  # total inverter AC output cap; 0 = unconstrained
INVERTER_AC_CAPACITY_MARGIN = 0.05  # model-side haircut on the AC cap (0-1)
...
class BatterySettings:
    ...
    inverter_ac_capacity_kw: float = INVERTER_AC_CAPACITY_KW
    inverter_ac_capacity_margin: float = INVERTER_AC_CAPACITY_MARGIN
    ...
    def __post_init__(self):
        ...
        if self.inverter_ac_capacity_kw < 0:
            raise ValueError(
                f"inverter_ac_capacity_kw must be >= 0 (0 disables the AC cap), "
                f"got {self.inverter_ac_capacity_kw}"
            )
        if not 0 <= self.inverter_ac_capacity_margin < 1:
            raise ValueError(
                f"inverter_ac_capacity_margin must be in [0, 1), "
                f"got {self.inverter_ac_capacity_margin}"
            )
